...
public class ResultController implements Initializable {
        ...
        for (RuleHuman r : rules) {
            if (r.desease.equals(diagnose)) {
                if (medicinesForDiagnose != null) {
                    FXUtils.warningAlert("Ошибка", "Для указанного диагноза найдено нескыолько одинаковых и рекомендуемых медикамента.");
                    return;
                }
        ...
        crosses.forEach((key, value) -> {
            ...
            value.forEach((sit) -> {
                sb.append(String.format(
                        "Ид. %d, Диаг. %s, ФИО %s, Рост %d, Вес %d, Пол %s, Возр. %d, План %s, Факторы %s, Соп. %s, Спец. %s\n",
    ...
    private int getWeightCategory(long weight, long growth) {
        int index = (int) (weight / Math.pow(growth, 2));
